...
- **Dynamic Configuration**: On every discovery change (a new process observed,
  ...
  **Runtime Service Tracking (RFD 053)**; see chapter 16 (Process & Service
  Discovery and Naming) for how the candidate set feeding this generation is
  produced, merged, and named.
...
### Process Discovery, Namespace-Aware Scanning, and Naming

How candidate processes are found (RFD 102), how listeners inside container
network namespaces are discovered (RFD 112), how default-on observation
feeds the service map via OTLP (RFD 103), and how a `ServiceEntry` gets its
name (RFD 104) are covered in their own chapter — see **chapter 16: Process
& Service Discovery and Naming**. That chapter is the producer for the
`ProcessCandidate` list `generateBeylaConfig` (above) consumes.
...
Process discovery, namespace-aware scanning, and naming have their own
Future Engineering Note entries — see chapter 16 (Metric-Triggered Service
Observation, Retroactive Naming Conflict Resolution, and Container-Aware
Discovery Providers).
