...
coral query cpu-profile --service api --since 1h --format svg > cpu.svg     # Generate flame graph
...
coral query memory-profile --service api --since 1h --format svg > memory.svg                    # SVG flame graph
```
...
coral profile cpu --service <name> [--duration <seconds>] [--frequency <hz>] [--format folded|json|svg] [--pod <name>]
...
coral profile cpu --service api --format svg > cpu.svg        # Generate interactive SVG flame graph
...
coral profile memory --service api --format svg > memory.svg  # Generate interactive SVG flame graph
...
#   --format <type>        Output format: folded (default), json, svg
```
