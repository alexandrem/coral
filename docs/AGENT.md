...
- **Description**: Address to bind the OTLP gRPC receiver. Use `0.0.0.0` to
  listen on all interfaces or `127.0.0.1` for localhost only.
- **Format**: `"<host>:<port>"`
...
- **Description**: Address to bind the OTLP HTTP receiver. Use `0.0.0.0` to
  listen on all interfaces or `127.0.0.1` for localhost only.
- **Format**: `"<host>:<port>"`
...
- **Type**: `integer`
- **Default**: `1`
- **Description**: How long to keep spans in local storage (in hours). Older
  spans are automatically deleted.
- **Rationale**: Pull-based architecture - colony queries recent data for AI
  investigations.
...
- **Type**: `float`
- **Default**: `500.0`
- **Description**: Latency threshold in milliseconds. Spans with duration
  exceeding this value are always captured.
...
- **Default**: `0.10`
- **Description**: Sampling rate for normal spans (not errors, not high
  latency).
...
# Check span count
duckdb /var/lib/coral-agent/telemetry.db "SELECT COUNT(*) FROM otel_spans_local;"
```
