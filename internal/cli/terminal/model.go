...
func (m TerminalModel) Init() tea.Cmd {
	cmds := []tea.Cmd{m.main.Init(), m.sidebar.Init()}
	if m.server != nil {
		cmds = append(cmds, waitForFollowupCmd(m.server))
	}
	return tea.Batch(cmds...)
}

// followupMsg carries a follow-up question submitted from the browser
// dashboard (a table row, bar, or timeseries point click).
type followupMsg struct{ question string }

// waitForFollowupCmd blocks on the server's follow-up channel and converts
// the next question into a Bubbletea message. Returns nil once the server is
// gone (e.g. on shutdown), ending the listen loop.
func waitForFollowupCmd(srv *Server) tea.Cmd {
	return func() tea.Msg {
		q, ok := <-srv.Followups()
		if !ok {
			return nil
		}
		return followupMsg{question: q}
	}
}
...
func (m TerminalModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case followupMsg:
		// Pre-fill (but don't submit) the question generated by a dashboard
		// click, and bring the main pane into focus so the user sees it.
		m.main = m.main.SetInputValue(msg.question)
		m.focus = focusMain
		cmds = append(cmds, waitForFollowupCmd(m.server))
