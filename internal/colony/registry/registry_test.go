...
func TestRegistry_Register(t *testing.T) {
	...
	t.Run("successful registration", func(t *testing.T) {
		entry, err := reg.Register("agent-1", "frontend", "10.42.0.2", "fd42::2")
	...
	t.Run("empty agent_id", func(t *testing.T) {
		_, err := reg.Register("", "frontend", "10.42.0.2", "fd42::2")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "agent_id cannot be empty")
	})

	t.Run("empty component_name", func(t *testing.T) {
		_, err := reg.Register("agent-1", "", "10.42.0.2", "fd42::2")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "component_name cannot be empty")
	})
	...
	t.Run("update existing registration", func(t *testing.T) {
		...
		entry1, err := reg.Register("agent-1", "frontend", "10.42.0.2", "fd42::2")
		require.NoError(t, err)
		...
		entry2, err := reg.Register("agent-1", "frontend-v2", "10.42.0.3", "fd42::3")
		require.NoError(t, err)
...
func TestRegistry_Get(t *testing.T) {
	...
	t.Run("get existing agent", func(t *testing.T) {
		_, err := reg.Register("agent-1", "frontend", "10.42.0.2", "fd42::2")
		require.NoError(t, err)
...
func TestRegistry_ListAll(t *testing.T) {
	...
	t.Run("multiple agents", func(t *testing.T) {
		...
		reg.Register("agent-3", "worker", "10.42.0.4", "fd42::4")
...
func TestRegistry_CountActive(t *testing.T) {
	...
	t.Run("mixed status agents", func(t *testing.T) {
		...
		// Register agents.
		reg.Register("agent-healthy", "frontend", "10.42.0.2", "fd42::2")
		reg.Register("agent-degraded", "api", "10.42.0.3", "fd42::3")
		reg.Register("agent-unhealthy", "worker", "10.42.0.4", "fd42::4")
...
func TestRegistry_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(id int) {
			reg.Register("agent-concurrent", "frontend", "10.42.0.2", "fd42::2")
			done <- true
		}(i)
	}
