...
// TestExecuteShellStartTool tests the coral_shell_start MCP tool (RFD 045).
func TestExecuteShellStartTool(t *testing.T) {
	logger := logging.NewWithComponent(logging.Config{
		Level:  "error",
		Pretty: false,
	}, "mcp-test")

	t.Run("shell start - healthy agent", func(t *testing.T) {
		reg := registry.New()

		services := []*meshv1.ServiceInfo{{Name: "api-server", Port: 8080}}
		_, err := reg.Register("agent-api-1", "api", "10.100.0.5", "fd42::5", services, nil, "v1.0.0")
		require.NoError(t, err)
		...
		result, err := server.executeShellStartTool(nil, input)
		require.NoError(t, err)

		// Verify response contains expected fields.
		assert.Contains(t, result, "Shell Access Available: api-server")
		assert.Contains(t, result, "Agent ID: agent-api-1")
		assert.Contains(t, result, "Service: api-server")
		assert.Contains(t, result, "Mesh IP: 10.100.0.5")
		assert.Contains(t, result, "Status: healthy")
		assert.Contains(t, result, "Connection Command:")
		assert.Contains(t, result, "coral shell --agent-addr 10.100.0.5:9001")
		assert.Contains(t, result, "⚠️  Warning: Agent shells have elevated privileges:")
		assert.Contains(t, result, "Available Utilities:")
		assert.Contains(t, result, "Shell: /bin/bash")

		// Should NOT contain degraded or unhealthy warnings for healthy agent.
		assert.NotContains(t, result, "Agent is in degraded state")
		assert.NotContains(t, result, "Agent is unhealthy")
	})

	t.Run("shell start - with agent_id", func(t *testing.T) {
		reg := registry.New()
		...
		_, err := reg.Register("agent-api-1", "api", "10.100.0.5", "fd42::5", services1, nil, "v1.0.0")
		require.NoError(t, err)
		_, err = reg.Register("agent-api-2", "api", "10.100.0.6", "fd42::6", services2, nil, "v1.0.0")
		require.NoError(t, err)
		...
		// Use agent_id to disambiguate.
		input := `{"service": "api", "agent_id": "agent-api-2"}`
		result, err := server.executeShellStartTool(nil, input)
		require.NoError(t, err)

		assert.Contains(t, result, "Agent ID: agent-api-2")
		assert.Contains(t, result, "Mesh IP: 10.100.0.6")
		assert.Contains(t, result, "coral shell --agent-addr 10.100.0.6:9001")
	})

	t.Run("shell start - custom shell", func(t *testing.T) {
		reg := registry.New()
		...
		input := `{"service": "api-server", "shell": "/bin/sh"}`
		result, err := server.executeShellStartTool(nil, input)
		require.NoError(t, err)

		assert.Contains(t, result, "coral shell --agent-addr 10.100.0.5:9001 --shell /bin/sh")
		assert.Contains(t, result, "Shell: /bin/sh")
	})

	t.Run("shell start - agent not found", func(t *testing.T) {
		reg := registry.New()
		...
		input := `{"service": "nonexistent-service"}`
		result, err := server.executeShellStartTool(nil, input)
		assert.Error(t, err)
	...
	t.Run("shell start - multiple agents (disambiguation error)", func(t *testing.T) {
		reg := registry.New()
		...
		assert.Empty(t, result)
		assert.Contains(t, err.Error(), "multiple agents found for service 'api'")
		assert.Contains(t, err.Error(), "agent-api-1, agent-api-2")
		assert.Contains(t, err.Error(), "Please specify agent_id parameter to disambiguate")
	})

	t.Run("shell start - multi-service agent", func(t *testing.T) {
		reg := registry.New()
		...
		_, err := reg.Register("agent-multi", "multi", "10.100.0.7", "fd42::7", services, nil, "v1.0.0")
		require.NoError(t, err)
		...
		// Should show all services in the response.
		assert.Contains(t, result, "Shell Access Available: api, worker")
		assert.Contains(t, result, "Service: api, worker")
		assert.Contains(t, result, "Agent ID: agent-multi")
	})

	t.Run("shell start - invalid JSON", func(t *testing.T) {
		reg := registry.New()
